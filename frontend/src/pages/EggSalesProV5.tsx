...
import {
  Camera, CheckCircle2, Copy, Download, Edit3, ExternalLink, Facebook,
  Image as ImageIcon, Loader2, Mail, Megaphone, MessageCircle, Package,
  PauseCircle, PlayCircle, Plus, Share2, ShoppingBasket, Store, Trash2,
  Upload, Users, Wallet, Wand2
} from 'lucide-react';
...
function slugify(value: string) {
  return value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/å/g, 'a').replace(/ä/g, 'a').replace(/ö/g, 'o').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60) || 'farska-agg';
}
function safeNumber(value: string, fallback = 0) { const n = Number(value); return Number.isFinite(n) ? n : fallback; }
function encode(value: string) { return encodeURIComponent(value.trim()); }
function getExt(file: File) { const ext = file.name.split('.').pop()?.toLowerCase(); return ext && ['jpg', 'jpeg', 'png', 'webp', 'heic'].includes(ext) ? ext : file.type.includes('png') ? 'png' : file.type.includes('webp') ? 'webp' : 'jpg'; }
function kr(value: unknown) { const n = Number(value || 0); return `${Math.round(n)} kr`; }
function statusLabel(status: string) { return status === 'paid' ? 'Betald' : status === 'picked_up' ? 'Hämtad' : status === 'cancelled' ? 'Avbokad' : 'Bokad'; }
function copyText(text: string, label = 'Texten') { navigator.clipboard?.writeText(text); toast({ title: `${label} är kopierad` }); }
...
async function getUniqueSlug(baseSlug: string, currentId?: string | null) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const { data, error } = await (supabase as any).from('public_egg_sale_listings').select('id').eq('slug', candidate).maybeSingle();
...
export default function EggSalesProV5() {
  ...
  const { data: listings = [], isLoading: listingsLoading } = useQuery({
    queryKey: ['my-public-egg-sale-listings-v5'],
    queryFn: async () => {
      const userId = await getCurrentUserId();
      const { data, error } = await (supabase as any).from('public_egg_sale_listings').select('*').eq('user_id', userId).order('updated_at', { ascending: false });
  ...
  const { data: bookings = [], isLoading: bookingsLoading } = useQuery({
    queryKey: ['my-public-egg-sale-bookings-v5'],
    queryFn: async () => {
      const userId = await getCurrentUserId();
  ...
  const bookingCounts = useMemo(() => {
    ...
    (bookings as Booking[]).forEach((b) => { if (b.status !== 'cancelled') map[b.listing_id] = (map[b.listing_id] || 0) + Number(b.packs || 0); });
    return map;
  }, [bookings]);

  const bookingStats = useMemo(() => {
    const active = (bookings as Booking[]).filter((b) => b.status !== 'cancelled');
    const reserved = active.filter((b) => b.status === 'reserved').length;
    const paid = active.filter((b) => b.status === 'paid' || b.status === 'picked_up').length;
    const packs = active.reduce((sum, b) => sum + Number(b.packs || 0), 0);
    const amount = active.reduce((sum, b) => {
      const listing = listingById[b.listing_id];
      return sum + Number(b.packs || 0) * Number(listing?.price_per_pack || 0);
    }, 0);
    return { total: active.length, reserved, paid, packs, amount };
  ...
  const loadListing = (l: Listing) => {
    setEditingId(l.id); setPublishedSlug(l.slug || ''); setCustomSlug(l.slug || ''); setTitle(l.title || 'Färska ägg till salu'); setDescription(l.description || ''); setImageUrl(l.image_url || '');
    setSalePacks(String(l.packs_available || 1)); setPackSize(String(l.eggs_per_pack || 12)); setPricePerPack(String(Math.round(Number(l.price_per_pack || 60))));
    setLocation(l.location || ''); setPickupInfo(l.pickup_info || ''); setContact(l.contact_info || ''); setSwishNumber(l.swish_number || ''); setSwishName(l.swish_name || ''); setSwishMessage(l.swish_message || 'Ägg'); setGeneratedTexts(null);
  ...
  const newListing = () => { setEditingId(null); setPublishedSlug(''); setCustomSlug(''); setGeneratedTexts(null); toast({ title: 'Ny säljlista skapad' }); };
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      setImageUrl(data.publicUrl); setGeneratedTexts(null); toast({ title: 'Bilden är uppladdad 📸' });
    } catch (e: any) { toast({ title: 'Kunde inte ladda upp bild', description: e.message, variant: 'destructive' }); }
  ...
  const publishListing = async () => {
    ...
    try {
      ...
      const row = { user_id: userId, slug, title: title.trim(), description: description.trim(), image_url: imageUrl.trim() || null, packs_available: packCount, eggs_per_pack: eggsPerPack, price_per_pack: price, location: location.trim() || null, pickup_info: pickupInfo.trim() || null, contact_info: contact.trim() || null, swish_number: swishNumber.trim() || null, swish_name: swishName.trim() || null, swish_message: swishMessage.trim() || 'Ägg', p6_price: p6, p12_price: p12, p30_price: p30, is_active: true };
      const result = editingId ? await (supabase as any).from('public_egg_sale_listings').update(row).eq('id', editingId).select('id, slug').single() : await (supabase as any).from('public_egg_sale_listings').insert(row).select('id, slug').single();
      if (result.error) throw result.error;
      setEditingId(result.data.id); setPublishedSlug(result.data.slug); setCustomSlug(result.data.slug);
      await qc.invalidateQueries({ queryKey: ['my-public-egg-sale-listings-v5'] });
    ...
    } catch (e: any) { toast({ title: 'Kunde inte publicera', description: e.message, variant: 'destructive' }); }
  ...
  const updateListingState = async (id: string, patch: Record<string, unknown>, message: string) => {
    ...
    await qc.invalidateQueries({ queryKey: ['my-public-egg-sale-listings-v5'] }); toast({ title: message });
  };
  ...
  const deleteListing = async (listing: Listing) => {
    ...
    await qc.invalidateQueries({ queryKey: ['my-public-egg-sale-listings-v5'] });
    await qc.invalidateQueries({ queryKey: ['my-public-egg-sale-bookings-v5'] });
  ...
  const updateBookingStatus = async (id: string, status: string) => {
    ...
    await qc.invalidateQueries({ queryKey: ['my-public-egg-sale-bookings-v5'] }); toast({ title: 'Bokningen är uppdaterad' });
  };

  const generateWithAI = async () => {
    setAiLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke('egg-sales-marketing', { body: { salePacks, packSize, pricePerPack, location, contact: `${contact}\nHämtning: ${pickupInfo}\nSwish: ${swishNumber || 'ej angivet'} ${swishName || ''}`, tone, sellingPoint: description, monthEggs: packCount * eggsPerPack, monthFeedCost: 0, costPerEgg: 0, suggestedLow: price, suggestedHigh: Math.ceil(price * 1.2) } });
      if (error) throw error;
      setGeneratedTexts({ ...data, facebook: `${data.facebook}\n\nSkicka bokningsförfrågan här: ${saleUrl}` }); toast({ title: 'Agda har skapat säljtexter ✨' });
    } catch (e: any) { toast({ title: 'Kunde inte skapa AI-texter', description: e.message, variant: 'destructive' }); }
    finally { setAiLoading(false); }
  };

  const shareNative = async () => { if (navigator.share) await navigator.share({ title, text: texts.facebook, url: saleUrl }).catch(() => undefined); else copyText(texts.facebook, 'Delningstexten'); };
  ...
  return <div className="max-w-7xl mx-auto space-y-5 pb-8 animate-fade-in">
    <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-3">
      <div><p className="data-label mb-1">Sälj ägg</p><h1 className="text-2xl sm:text-3xl font-serif text-foreground">Agda-säljgenerator 🥚</h1><p className="text-sm text-muted-foreground mt-1">Skapa säljlistor, ta emot bokningsförfrågningar, visa Swish och håll koll på lagret.</p></div>
      <div className="flex flex-col sm:flex-row gap-2"><Button variant="outline" onClick={newListing} className="rounded-xl gap-2"><Plus className="h-4 w-4" /> Ny lista</Button><Button onClick={publishListing} disabled={publishing} className="rounded-xl gap-2">{publishing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Store className="h-4 w-4" />} {editingId ? 'Uppdatera' : 'Publicera'}</Button></div>
    </div>
    ...
    <Card className="border-primary/25 bg-gradient-to-br from-primary/10 via-card to-accent/10 shadow-sm">
      <CardContent className="p-4 sm:p-5 space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          <div><h2 className="font-serif text-lg flex items-center gap-2"><ShoppingBasket className="h-5 w-5 text-primary" /> Beställningar / bokningsförfrågningar</h2><p className="text-sm text-muted-foreground">Här syns alla som skickat in en förfrågan via dina säljsidor.</p></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Button variant="outline" className="rounded-xl gap-2" onClick={copyCustomerList}><Copy className="h-4 w-4" /> Kopiera kundlista</Button><Button variant="outline" className="rounded-xl gap-2" onClick={exportBookingsCsv}><Download className="h-4 w-4" /> Exportera CSV</Button></div>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <MiniStat label="Förfrågningar" value={bookingStats.total} />
          <MiniStat label="Obekräftade" value={bookingStats.reserved} />
          <MiniStat label="Kartor" value={bookingStats.packs} />
          <MiniStat label="Värde" value={kr(bookingStats.amount)} />
        </div>
        {bookingsLoading ? <div className="py-6 flex justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div> : (bookings as Booking[]).length === 0 ? <div className="rounded-2xl border border-dashed p-5 text-center"><Users className="h-8 w-8 mx-auto text-muted-foreground mb-2" /><p className="font-medium">Inga bokningsförfrågningar ännu</p><p className="text-sm text-muted-foreground mt-1">När någon beställer via din publika länk visas det här direkt.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">{(bookings as Booking[]).map((b) => {
          const listing = listingById[b.listing_id];
          const unitPrice = Number(listing?.price_per_pack || 0);
          const total = unitPrice ? Number(b.packs || 0) * unitPrice : 0;
          return <div key={b.id} className="rounded-2xl border bg-card p-4 space-y-3 shadow-sm"><div className="flex justify-between gap-2"><div className="min-w-0"><p className="font-medium text-sm truncate">{b.customer_name}</p><p className="text-xs text-muted-foreground truncate">{b.customer_phone || 'Ingen kontakt angiven'}</p></div><Badge>{b.packs} kartor</Badge></div><div className="rounded-xl bg-muted/30 border p-3 text-xs space-y-1"><p><span className="text-muted-foreground">Gäller:</span> <strong>{listing?.title || 'Okänd säljlista'}</strong></p>{listing?.slug && <p className="truncate text-muted-foreground">{PUBLIC_BASE_URL}/s/{listing.slug}</p>}<p><span className="text-muted-foreground">Summa:</span> <strong>{total ? kr(total) : 'Pris saknas'}</strong></p></div>{b.customer_message && <p className="text-xs text-muted-foreground whitespace-pre-wrap">{b.customer_message}</p>}<Badge variant="outline">{statusLabel(b.status)}</Badge><div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" className="rounded-xl" onClick={() => updateBookingStatus(b.id, 'paid')}><CheckCircle2 className="h-3.5 w-3.5 mr-1" /> Betald</Button><Button size="sm" variant="outline" className="rounded-xl" onClick={() => updateBookingStatus(b.id, 'picked_up')}>Hämtad</Button><Button size="sm" variant="outline" className="rounded-xl col-span-2" onClick={() => copyText(customerReply(b), 'Svar till kund')}><MessageCircle className="h-3.5 w-3.5 mr-1" /> Kopiera svar till kund</Button><Button size="sm" variant="ghost" className="rounded-xl text-destructive hover:text-destructive col-span-2" onClick={() => updateBookingStatus(b.id, 'cancelled')}><Trash2 className="h-3.5 w-3.5 mr-1" /> Avboka</Button></div></div>;
        })}</div>}
      </CardContent>
    </Card>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
      <div className="xl:col-span-2 space-y-5">
        <Card className="border-primary/25 bg-gradient-to-br from-primary/10 via-card to-accent/10 shadow-sm"><CardContent className="p-4 sm:p-5 space-y-4">
          ...
          <div className="rounded-2xl border border-border/50 bg-card/70 p-4 space-y-3"><p className="font-serif text-sm flex items-center gap-2"><Camera className="h-4 w-4 text-primary" /> Bild</p>{imageUrl ? <img src={imageUrl} className="h-52 w-full rounded-2xl object-cover border" /> : <div className="h-40 rounded-2xl border border-dashed flex items-center justify-center text-muted-foreground"><ImageIcon className="h-8 w-8" /></div>}<div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><FileButton label="Ta ny bild" icon="camera" uploading={imageUploading} capture onFile={uploadImage} /><FileButton label="Välj från galleri" icon="upload" uploading={imageUploading} onFile={uploadImage} /></div><Input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} placeholder="Eller klistra in bildlänk" className="rounded-xl" /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3"><Field label="Kartor" value={salePacks} setValue={setSalePacks} type="number" /><Field label="Ägg/karta" value={packSize} setValue={setPackSize} type="number" /><Field label="Pris/karta" value={pricePerPack} setValue={setPricePerPack} type="number" /><label className="space-y-1.5"><span className="text-xs text-muted-foreground">Ton</span><select value={tone} onChange={(e) => setTone(e.target.value as Tone)} className="w-full h-11 rounded-xl border border-input bg-background px-3 text-sm"><option value="mysig">Mysig</option><option value="kort">Kort</option><option value="proffsig">Proffsig</option><option value="humor">Humor</option></select></label><Field label="Plats" value={location} setValue={setLocation} /><Field label="Önskad länk" value={customSlug} setValue={(v) => setCustomSlug(slugify(v))} placeholder={draftSlug} /><Field label="Hämtning" value={pickupInfo} setValue={setPickupInfo} /><Field label="Kontakt" value={contact} setValue={setContact} /><Field label="Swishnummer" value={swishNumber} setValue={setSwishNumber} /><Field label="Swishnamn" value={swishName} setValue={setSwishName} /><Field label="Swish-meddelande" value={swishMessage} setValue={setSwishMessage} /></div>
...
function Field({ label, value, setValue, placeholder, type = 'text' }: { label: string; value: string; setValue: (v: string) => void; placeholder?: string; type?: string }) { return <label className="space-y-1.5"><span className="text-xs text-muted-foreground">{label}</span><Input type={type} value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} className="rounded-xl h-11" /></label>; }
function MiniStat({ label, value }: { label: string; value: string | number }) { return <div className="rounded-xl bg-muted/30 border p-3"><p className="font-bold text-foreground tabular-nums">{value}</p><p className="text-[9px] uppercase tracking-widest text-muted-foreground mt-1">{label}</p></div>; }
function PriceRow({ label, value }: { label: string; value: string }) { return <div className="flex justify-between"><span className="text-muted-foreground">{label}</span><span className="font-semibold">{value}</span></div>; }
function MarketingOutput({ icon: Icon, title, text }: { icon: any; title: string; text: string }) { return <Card className="border-border/50 bg-card/80 shadow-sm"><CardContent className="p-4 space-y-3"><div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2 min-w-0"><Icon className="h-4 w-4 text-primary shrink-0" /><h3 className="font-serif text-sm truncate">{title}</h3></div><Button variant="ghost" size="sm" onClick={() => copyText(text, title)} className="h-8 px-2 rounded-lg"><Copy className="h-3.5 w-3.5 mr-1" /> Kopiera</Button></div><Textarea value={text} readOnly className="min-h-[120px] rounded-xl text-xs leading-relaxed resize-none bg-muted/25" /></CardContent></Card>; }

...
import { ArrowRight, Bell, Bird, CalendarDays, CheckCircle2, Copy, Egg, PackageCheck, ReceiptText, Sparkles, TrendingDown, TrendingUp, Wallet, Wheat } from 'lucide-react';
...
export default function SmartFarmReport() {
  ...
  useEffect(() => {
    document.title = 'Smart gårdsrapport | Hönsgården';
  }, []);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5 pb-8 animate-fade-in">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-3">
        <div>
          <p className="data-label mb-1">SaaS-rapport</p>
          <h1 className="text-2xl sm:text-3xl font-serif text-foreground">Smart gårdsrapport ✨</h1>
          <p className="text-sm text-muted-foreground mt-1">{weekLabel} · ägg, flock, foder, uppgifter och Agdas Bod i samma överblick.</p>
        </div>
        <Button className="rounded-xl gap-2" onClick={() => copyText(plainReport, 'Gårdsrapporten')}>
          <Copy className="h-4 w-4" /> Kopiera rapport
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card className="xl:col-span-2 border-primary/20 bg-gradient-to-br from-primary/8 via-card to-accent/5 shadow-sm">
          <CardContent className="p-4 sm:p-5 space-y-4">
            <div className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-primary" />
              <h2 className="font-serif text-lg text-foreground">Agdas smarta sammanfattning</h2>
            </div>
            <div className="space-y-3">
              {report.insights.map((insight, index) => (
                <div key={index} className="rounded-2xl border bg-background/70 p-4 text-sm leading-relaxed text-foreground">
                  {insight}
                </div>
              ))}
            </div>
        ...
        <Card className="shadow-sm">
          <CardContent className="p-4 sm:p-5 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <h2 className="font-serif text-lg text-foreground">Nästa bästa steg</h2>
              <Badge variant="secondary">Smart</Badge>
            </div>
            {actions.length === 0 ? (
              <div className="rounded-2xl border border-success/20 bg-success/5 p-4">
                <p className="font-medium text-sm text-foreground">Bra jobbat!</p>
                <p className="text-xs text-muted-foreground mt-1">Du har redan fyllt rapporten med bra data den här veckan.</p>
              </div>
            ) : actions.map((action) => (
              <button key={action.title} onClick={() => navigate(action.path)} className="w-full rounded-2xl border bg-card hover:bg-muted/30 p-3 text-left flex items-center gap-3 transition-colors">
                <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><action.icon className="h-4 w-4 text-primary" /></div>
                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-foreground">{action.title}</p><p className="text-xs text-muted-foreground">{action.text}</p></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <MiniPanel icon={Wheat} title="Foder" value={report.feedCostPerEgg ? `${Number(report.feedCostPerEgg).toFixed(2)} kr/ägg` : 'Saknas'} text="Lägg in foderköp för att göra rapporten mer ekonomisk." />
        <MiniPanel icon={ReceiptText} title="Säljsidor" value={report.activeListings} text="Aktiva säljsidor i Agdas Bod just nu." />
        <MiniPanel icon={CalendarDays} title="Veckorytm" value={report.weekEggs > 0 ? 'Igång' : 'Starta'} text="Rapporten blir starkare när du loggar lite varje vecka." />
      </div>
    </div>
  );
}
    ...
    <Card className="shadow-sm">
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          {TrendIcon && <TrendIcon className={`h-4 w-4 ${trend >= 0 ? 'text-success' : 'text-destructive'}`} />}
        </div>
  ...
  return (
    <Card className="border-border/60 shadow-sm">
      <CardContent className="p-4 flex gap-3">
        <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><Icon className="h-4 w-4 text-primary" /></div>
        <div>

...
function extractFeedbackId(text: string) {
  const match = text.match(/(?:feedback|förslag|synpunkt)[\s:#-]*([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i);
  return match?.[1] ?? null;
}

function getNotificationTarget(notification: Notification) {
  const combinedRaw = `${notification.title} ${notification.message}`;
  ...
  const feedbackId = extractFeedbackId(combinedRaw);
  if (feedbackId) return `/app/community?feedback=${feedbackId}`;
  ...
  return (
    <Popover open={open} onOpenChange={handleOpen}>
      <PopoverTrigger asChild>
        <button className="relative p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" aria-label={unreadCount > 0 ? `Visa notiser, ${unreadCount} olästa` : 'Visa notiser'}>
          <Bell className="h-5 w-5" />
          {unreadCount > 0 && <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-bold text-destructive-foreground animate-pulse">{unreadCount > 9 ? '9+' : unreadCount}</span>}
        </button>
      </PopoverTrigger>
        ...
        <ScrollArea className="max-h-[70vh] sm:max-h-[28rem]">
          {notifications.length === 0 ? (
            ...
            <div className="divide-y divide-border/40">
              {notifications.map(n => (
                <article key={n.id} className={`px-4 py-4 transition-colors ${!readIds.has(n.id) ? 'bg-primary/5' : ''}`}>
                  <p className="text-sm sm:text-base font-medium text-foreground leading-snug break-words">{n.title}</p>
                  <p className="text-xs sm:text-sm text-muted-foreground mt-1.5 whitespace-pre-wrap break-words leading-relaxed">{n.message}</p>
                  <div className="mt-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <p className="text-[10px] sm:text-xs text-muted-foreground/60">{formatDistanceToNow(new Date(n.created_at), { addSuffix: true, locale: sv })}</p>
                    <button type="button" onClick={() => handleReadMore(n)} className="inline-flex w-full sm:w-auto items-center justify-center gap-1.5 rounded-xl bg-primary/10 px-3 py-2 text-xs font-medium text-primary hover:bg-primary/15 transition-colors active:scale-[0.98]">
